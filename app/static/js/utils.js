/* ============================================================================
   Utils — Versão global (UMD) sem exports (compat <script> clássico)
   - API exposta: window.utils e também nomes soltos no window (getCSRFToken etc.)
...
(function (global) {
  ...
  function withCSRFFromMeta(headers = new Headers()) {
  ...
  function mostrarMensagem(msg, tipo = 'sucesso', timeout = 5000, elId = 'mensagem-feedback') {
  ...
  function mostrarLoading(show = true, elId = 'loading-spinner') {
    const el = typeof document !== 'undefined' && document.getElementById(elId);
    if (!el) return;
    if (show) {
  ...
  function atualizarProgresso(percent, containerId = 'progress-container', barId = 'progress-bar') {
  ...
  function resetarProgresso(containerId = 'progress-container', barId = 'progress-bar') {
    const container = typeof document !== 'undefined' && document.getElementById(containerId);
    const bar = typeof document !== 'undefined' && document.getElementById(barId);
    if (!container || !bar) return;
    bar.style.width = '0%';
   ...
   */
  function getThumbWidth(containerOrSel = null, fallback = 200) {
    try {
      const el =
  ...
  function getMediaSize(el) {
    ...
    if (global.HTMLCanvasElement && el instanceof HTMLCanvasElement) {
      const w = Number(el.width) || 0;
      const h = Number(el.height) || 0;
      return (w && h) ? { w, h } : null;
    }
    if (global.HTMLImageElement && el instanceof HTMLImageElement) {
  ...
  function containScale(containerW, containerH, contentW, contentH) {
   ...
   */
  function fitRotateMedia({ frameEl, mediaEl, angle = 0 } = {}) {
  ...
  function getCropBoxAbs(wrapperEl) {
   ...
   */
  function collectPagesRotsCropsAllOrSelection(gridEl) {
    if (!gridEl) return { pages: [], rotations: [], crops: [] };

    // 1) tenta util público do preview.js (se existir)
    let pages = [];
    try {
      const fn = (global.getSelectedPages || (global.preview && global.preview.getSelectedPages));
  ...
  global.utils = api;
  // nomes soltos (para scripts legados)
  Object.keys(api).forEach((k) => { try { global[k] = api[k]; } catch {} });
